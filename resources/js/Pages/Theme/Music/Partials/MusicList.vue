...
const page = usePage<
    {
        data?: PaginatedResponseData<Music>;
...
const selectedContent = ref<Music | null>(null);
...
const deleteContent = (item: Music) => {
...
<template>
    <div>
        ...
        <div class="row">
            <Deferred :data="['data']">
                ...
                <div
                    v-for="music in data?.data"
                    :key="music.id"
                    class="col-12 col-sm-6 col-md-4 col-xl-3 d-flex align-items-stretch"
                >
                    <MusicCard :music="music" v-on:destroy="deleteContent" />
                </div>
    ...
    <Modal id="add-music-modal" :show="isAdding" @close="closeAddModal">
        <AddMusicForm v-if="isAdding" v-on:close-modal="closeAddModal" />
    </Modal>

    <Modal :show="isDeleting" id="delete-music-modal" @close="closeDeleteModal">
        <DeleteMusic
            :music="selectedContent ?? undefined"
